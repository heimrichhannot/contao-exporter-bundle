...
abstract class AbstractExporter implements ExporterInterface
{
    ...
    public function export(ExporterModel $config = null, $entity = null, array $fields = [])
    {
        ...
        if (!$this->hasType($this->config->type)) {
            throw new ExportTypeNotSupportedException('Export type '.$this->config->type.' is not supported by export class '.static::class.' for export configuration '.$this->config->title.' (ID: '.$this->config->id.')');
        }
        ...
        $event = $this->dispatcher->dispatch(new BeforeExportEvent($entity, $fields, $fileName, $this, $fileDir), BeforeExportEvent::NAME);
    ...
    public function getEntities($pid)
    {
        ...
        foreach (deserialize($this->config->tableFieldsForExport, true) as $field) {
            ...
            if (false !== strpos($field, static::EXPORTER_RAW_FIELD_SUFFIX)) {
    ...
    public function prepareItemFields(Model $entity, array $fields = []): array
    {
        ...
        foreach ($fields as $fieldName) {
            ...
            if ('explanation' != $fieldData['inputType']) {
